...
public class FileIO
{
	...
	public SortedList<Double> year, scaledList;
	public SortedList<Integer> scaledListRounded;
	...
	FileIO() throws IOException
	{
		...
		year = new SortedList<Double>();
		scaledList = new SortedList<Double>();
		scaledListRounded = new SortedList<Integer>();
		...
		scaleList(year, scaledList);
		scaleListRounded(year, scaledListRounded);
	}
	
	private void scaleListRounded(SortedList<Double> oldList,
			SortedList<Integer> newList)
	{
		for(int i = 1; i<oldList.getLength(); i++)
		{
			newList.add((int) (100 * (oldList.getEntry(i) - yearLow) / (yearHigh - yearLow)));
		}
		
	}

	private void scaleList(SortedList<Double> oldList, SortedList<Double> newList)
	{
		for(int i = 1; i<oldList.getLength(); i++)
		{
			newList.add(100 * (oldList.getEntry(i) - yearLow) / (yearHigh - yearLow));
		}
	}


	//accounts for the missing data by interpolating the data that came before/after it
